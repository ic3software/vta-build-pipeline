#!/usr/bin/env bash
#
# Local mirror of .github/workflows/build.yml: builds the VTA / VTC / PNM
# release binaries and uploads them to Cloudflare R2 under both `latest/`
# and a versioned path (`<crate-version>-<git-short-sha>/`).
#
# Builds:
#   vta-seed  — vta-service with config-seed features (uploaded as vta/)
#   vta-k8s   — vta-service with k8s-secrets features  (uploaded as vta-k8s/)
#   vtc       — vtc-service                            (uploaded as vtc/)
...
cargo build --release --no-default-features \
  ...
  -p vta-service
cp target/release/vta target/release/vta-seed

echo "==> building vta-service (k8s-secrets)"
cargo build --release --no-default-features \
  --features "setup,vault-secrets,didcomm,rest,cli-synthesis" \
  -p vta-service

echo "==> building vtc-service"
cargo build --release --no-default-features \
...
for bin in target/release/vta-seed target/release/vta target/release/vtc target/release/pnm; do
...
upload "target/release/vta-seed" "s3://${R2_BUCKET}/vta/latest/vta"
upload "target/release/vta-seed" "s3://${R2_BUCKET}/vta/${vta_version}-${git_hash}/vta"
...
upload "target/release/vtc"      "s3://${R2_BUCKET}/vtc/latest/vtc"
upload "target/release/vtc"      "s3://${R2_BUCKET}/vtc/${vtc_version}-${git_hash}/vtc"
