...
///
/// `SessionBackend` is a sync trait, but the Azure SDK only exposes async
/// methods. The naive `tokio::runtime::Handle::current().block_on(...)`
/// **panics** when called from inside a current-thread tokio runtime —
/// exactly the runtime most CLI consumers use (`#[tokio::main]` without
/// `flavor = "multi_thread"`).
///
/// This helper spawns a dedicated OS thread carrying its own
/// `current_thread` runtime, runs the future there, and joins. Cost is one
/// thread per call — acceptable for human-rate session ops (login, refresh,
/// logout) which run at most a handful of times per CLI invocation.
/// Anything hotter should use the OS keyring backend instead.
#[cfg(all(feature = "azure-secrets", not(feature = "keyring")))]
